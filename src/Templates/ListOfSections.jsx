...
const ListOfSectionsTemplates = () => {
  ...
  useEffect(() => {
    const q = query(
      collection(db, "listOfSectionsTemplates"),
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      const filteredTemplates = querySnapshot.docs.filter((doc) => {
        ...
        return [data.name, data.description]
          .join(" ") // Added description to search
          .toLowerCase()
  ...
  const handleSaveTemplate = async () => {
    try {
      ...
      const templateData = {
        name,
        description, // Include description in saved data
        createdAt: serverTimestamp(),
      };
      await addDoc(collection(db, "listOfSectionsTemplates"), templateData);
  ...
  return (
    <div style={{ width: "100%" }}>
      ...
      >
        <DialogTitle>Add List of SectionsTemplate</DialogTitle>
        <DialogContent>
          <TextField
            error={name === ""}
...
export default ListOfSectionsTemplates;